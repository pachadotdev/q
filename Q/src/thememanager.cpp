...
{
    // Try multiple paths for the themes directory
    QStringList searchPaths;
    searchPaths << QCoreApplication::applicationDirPath() + "/../Q/themes";  // From build/bin to Q/themes
    searchPaths << QCoreApplication::applicationDirPath() + "/../themes";  // Development alt
    searchPaths << QCoreApplication::applicationDirPath() + "/../../Q/themes";  // Another build variant
    searchPaths << QCoreApplication::applicationDirPath() + "/../share/q/themes";  // Installed
    searchPaths << "/usr/share/q/themes";  // System-wide installation
    
    for (const QString &path : searchPaths) {
        QDir dir(path);
        if (dir.exists() && !dir.entryList(QStringList() << "*.json", QDir::Files).isEmpty()) {
            QString absPath = dir.absolutePath();
            // qDebug() << "Found themes directory:" << absPath;
            return absPath;
        }
    }
    
    qWarning() << "Themes directory not found in any location. Searched:";
    for (const QString &path : searchPaths) {
        qWarning() << "  -" << path;
    }
    return QString();
